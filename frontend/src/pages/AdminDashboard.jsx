...
export default function AdminDashboard() {
  ...
  const handleRequestPayment = async (orderId, trackingCode) => {
    if (!window.confirm(`Send a payment request notification to the student for order ${trackingCode}?`)) return;
    try {
      await requestPayment({ order_id: orderId });
      showToast('Payment request sent to student!'); loadAll();
    } catch (err) { showToast(err.response?.data?.message || 'Error sending request', 'error'); }
  };
                  ...
                  <tbody>
                    {data.orders.map(o => {
                      ...
                      <tr key={o.id} style={{ borderBottom:'1px solid rgba(255,255,255,0.03)' }}>
                        ...
                        <td style={{ padding:'12px 16px', display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                          {/* STEP 1: Submitted → assign worker */}
                          {o.status === 'submitted' && (
                            <>
